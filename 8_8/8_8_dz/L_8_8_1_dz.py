"""
В созданном классе “Параллелограмм” переопределить методы вычисления площади
и вывода информации о фигуре.
Написать программу, демонстрирующую работу с классом:
дано N параллелограммов, найти параллелограммы с наименьшей и наибольшей площадями.
"""
...
# Геттеры для получения точек с координатами четырёхугольника
    ...
    # для его дальнейшего переопределения в классе-потомке (28-02-2024)
    def get_area(self):
        """
    ...
    def get_info(self):
        ...
        :return:
        """
        print(f"Четырёхугольник с периметром {self.perimetr_quadrilateral()} и площадью {self.get_area()}")
...
class Parallelogram(Quadrilateral):
    ...
    # Переопределение метода из родительского класса (28-02-2024)
    def get_area(self, flag=False):
        """
        Вычисление площади пааллелограмма
        :param flag: если флаг True, то выводить на печать площадь параллелограмма
        :return: площадь параллелограмма
        """
        s = self.ABCD()[0] * self.ABCD()[1]
        if flag:
            print(f"Площадь параллелограмма равна: {s}")
        return s

# Переопределение метода из родительского класса (28-02-2024)
    def get_info(self):
        """
        Вывод на печать краткой информации о параллелограмме
        :return:
        """
        print(f"Параллелограмм со сторонами: {self.ABCD()[0]}, {self.ABCD()[1]}, {self.ABCD()[2]}, {self.ABCD()[3]}")
...
# q_s = list(map(lambda q: q.get_area(), quadrilaterals))
# if len(q_s) > 1:
...
# Вызов переопределённого в классе-потомке метода get_area() (28-02-2024)
print("*"*50)
p1.get_area(True)
print(p2.get_area())
print("*"*50)
...
min_p_s = min(parallelograms, key=lambda p: p.get_area())
# Получение объекта класса Parallelogram с максимальной площадью
max_p_s = max(parallelograms, key=lambda p: p.get_area())

# Вызов переопределённого в классе-потомке метода get_info() (28-02-2024)
